...
// Recommended fundamental constants of physics and chemistry based on the 2006 adjustment
 ...
 for(i=0;i<N_point;i++){
   ...
   // left side
   k_ome[MAXSTEP/2-N_point+i]= k_ome[MAXSTEP/2-N_point]-(k_ome[MAXSTEP/2-N_point]-k_ome[MAXSTEP/2])/(N_point)*(i);
   K[MAXSTEP/2-N_point+i]= K[MAXSTEP/2-N_point]-(K[MAXSTEP/2-N_point]-K[MAXSTEP/2])/(N_point)*(i);

   // right side
   k_ome[MAXSTEP/2+i]= k_ome[MAXSTEP/2+N_point]-(k_ome[MAXSTEP/2+N_point]-k_ome[MAXSTEP/2])/(N_point)*(N_point-i);
   K[MAXSTEP/2+i]= K[MAXSTEP/2+N_point]-(K[MAXSTEP/2+N_point]-K[MAXSTEP/2])/(N_point)*(N_point-i);
 ...
 for(step=0;step<MAXSTEP;step++){
   if (N_point != 0){
     if(step == (MAXSTEP/2-N_point+1)){mueff_mu[step]=(mueff_mu[step-2]+mueff_mu[step+2])/2.;}
     if(step == (MAXSTEP/2-N_point)){mueff_mu[step]=(mueff_mu[step-2]+mueff_mu[step+2])/2.;}
     if(step == (MAXSTEP/2+N_point)){mueff_mu[step]=(mueff_mu[step-2]+mueff_mu[step+2])/2.;}
     if(step == (MAXSTEP/2+N_point-1)){mueff_mu[step]=(mueff_mu[step-2]+mueff_mu[step+2])/2.;}
   }
 ...
 int Emax=calc_VaG(frequencies_save);
 int Emaxvar=Energy[MAXSTEP/2];
 if(isctvtst==1){
   for(i=0;i<MAXSTEP;i++){
...
void reading_inputfile () {
 ...
 //Updated, it is the energy barrier in kcal/mol wrt reacts without ZPE
  ...
  // Read saddlepbkf: parameter to decide how to estimate saddle point bkf
  if(!feof(fp)) {
    fscanf(fp,"%s %d",buffer,&saddlepbkf);
    //    printf("Saddle Point BKF:      %d\n", saddlepbkf);
  } else {
    printf("Data file error\nthe program will be stopped now\n");
    exit(0);
  }
...
void reading_pesrxfile () {
  ...
  for(i=0;i<MAXSTEP;i++){
    if(i <= (MAXSTEP-1)/2) Energy[i]=Energy[i]+deltar/627.5*((i-((MAXSTEP-1)/2.))/((MAXSTEP-1)/2.));
    //    if(i > (MAXSTEP-1)/2) Energy[i]=Energy[i]+deltap/627.5*((MAXSTEP-1)/2.-i)/((MAXSTEP-1)/2.);
    if(i > (MAXSTEP-1)/2) Energy[i]=Energy[i]+deltap/627.5*((MAXSTEP-1)/2.-i+1)/((MAXSTEP-1)/2.-1);
...
void reading_pesfile () {
  ...
  for(i=0;i<MAXSTEP;i++){
    if(i <= MAXSTEP/2) Energy[i]=Energy[i]+delta/627.5*((i-(MAXSTEP/2.))/(MAXSTEP/2.));

    if(i > MAXSTEP/2) Energy[i]=Energy[i]+delta/627.5*(MAXSTEP/2.-i)/(MAXSTEP/2.);
  }
...
double *calc_BkF(int dim,int step, double **L_int, double *BkF){
  ...
  if(step>((int)(MAXSTEP/2-npoints_rem))&& step<((int)(MAXSTEP/2+npoints_rem))){
...
int calc_VaG(double **frequencies_save){
  ...
  for(j=0;j<(int)(MAXSTEP/2-N_spline/2);j++){  
    E_int_temp[j]= E_temp[j];
  }
   
   
  for(j=(int)(MAXSTEP/2-N_spline/2);j<MAXSTEP-N_spline;j++){  
    E_int_temp[j]= E_temp[j+N_spline];
  }
      
  for(j=0;j<MAXSTEP-N_spline;j++){ 
    xE[j]=j;
    if(j>=(int)(MAXSTEP/2-N_spline/2)) xE[j]=j+N_spline;
  }
     
  for(j=(int)(MAXSTEP/2-N_spline/2);j<(int)(MAXSTEP/2+N_spline/2);j++){       
...
void deriv_gradient(void){
  ...
  for(j=0;j<MAXSTEP-N_spline;j++){
    ...
    x1[j]=j;
    if(j>=(int)(MAXSTEP/2-N_spline/2)) x1[j]=j+N_spline;
  }
...
for(i=0;i<3*ATOMS;i++){
 ...
 for(j=0;j<MAXSTEP-N_spline;j++){
   ...
   x[j]=j;
   if(j>=(int)(MAXSTEP/2-N_spline/2)) x[j]=j+N_spline;
 }
...
double calc_var_corr(int dim, double Trif, double **frequencies_save){
  ...
  tsflux=qvib[MAXSTEP/2]*qrot[MAXSTEP/2]*exp(-Energy[MAXSTEP/2]*1000./349.75/1.987/Trif);
  minflux=tsflux;
  stepmin=MAXSTEP/2;
